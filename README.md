...
С помощью вашего языка и его библиотек сделайте нагрузочное тестирование сервера из предыдущей лабораторной. Сколько подключений он может выдержать, как зависит RPS от размера файлов? Графики - было бы круто.

Для C# можно использовать https://nbomber.com/.

### Файловое хранилище (8 баллов)

Обычно файлы хранят не напрямую в файловой системе, а в специализированных хранилищах. Поднять minio. Из лабы про UPD файлы сохранять в minio. Рассказать по каким протоколам работает minio.

Для C# описание API клиента minio здесь - https://min.io/docs/minio/linux/developers/dotnet/API.html
