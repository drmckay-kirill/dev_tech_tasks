...
### Написать свой собственный класс для работы с мьютексами (10 баллов)
### Лучше делать на каком-нибудь высокоуровневом языке - C#, python, etc.
Задачи по решению проблем синхнронизации многопоточных приложений довольно часто встречаются на практике. Использовать семаформы/мьютексты разработчикам иногда приходится. В этом задании попробуем чуть-чуть посмотреть, как мьютексы сделаны изнутри.
...
Далее написанный мьютекс надо заиспользовать в предыдущем - 5 - задании как средство разграничения доступа. Только вместо 10 потоков использовать 10 отдельных процессов или экземпляров приложения.
